...
class NexraBrain:
    ...
    def providers_list(self): return self.providers
    def add_provider(self, name): self.providers.append(name)
    def capability(self, name): return {"gemini":"reasoning","openai":"general"}[name]
    def best(self, need): return next((p for p in self.providers if self.capability(p)==need),None)
    def fallback(self, need): return [p for p in self.providers if self.capability(p)==need]
    def available(self, name): return name in self.providers
